...
<html>
    ...
    $q_medkit1 = "SELECT medkit_2 FROM users WHERE user_fullname = '$current_user'";
    $result1 = mysqli_query($dbconn, $q_medkit1);

    if (mysqli_num_rows($result1)>0) {
        while($row = mysqli_fetch_assoc($result1)) {
            $medkit1 = $row["medkit_2"];

        } 
    }


    $q_drop = "DROP TABLE $medkit2";
    echo "$q_drop";
    mysqli_query($dbconn, $q_drop);

    echo "Medkit " .$medki21. " was deleted successfully";
